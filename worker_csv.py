...
def load_prompt_from_db(supabase_client, projeto_id=None):
    ...
    try:
        ...
        if result.data and result.data.get('prompt_text'):
            return result.data['prompt_text']
    except Exception as e:
        print(f"⚠️ Erro ao carregar prompt do Supabase: {e}")
    try:
        with open("prompt_custom.txt", "r", encoding="utf-8") as f:
            return f.read()
...
def process_file_task(record):
    ...
    with semaphore:  # Garante limite de threads ativas
        ...
        try:
            ...
            with open(r'e:\Projetos Cursor\ProcessIA\processia\.cursor\debug.log', 'a', encoding='utf-8') as f:
                import json as json_module
                f.write(json_module.dumps({"sessionId":"debug-session","runId":"run1","hypothesisId":"B","location":"worker_csv.py:process_file_task:loaded_prompt","message":"Prompt loaded from database","data":{"prompt_length":len(current_prompt) if current_prompt else 0,"has_chaves":current_prompt and "CHAVES OBRIGATÓRIAS" in current_prompt,"prompt_preview":current_prompt[:300] if current_prompt else None},"timestamp":int(time.time()*1000)}) + '\n')
            # #endregion
            ...
            with open(r'e:\Projetos Cursor\ProcessIA\processia\.cursor\debug.log', 'a', encoding='utf-8') as f:
                import json as json_module
                f.write(json_module.dumps({"sessionId":"debug-session","runId":"run1","hypothesisId":"A","location":"worker_csv.py:process_file_task:extracted_keys","message":"Expected keys from prompt","data":{"expected_keys":expected_keys,"count":len(expected_keys)},"timestamp":int(time.time()*1000)}) + '\n')
            # #endregion
            
            # #region agent log
            with open(r'e:\Projetos Cursor\ProcessIA\processia\.cursor\debug.log', 'a', encoding='utf-8') as f:
                f.write(json_module.dumps({"sessionId":"debug-session","runId":"run1","hypothesisId":"D","location":"worker_csv.py:process_file_task:before_filter","message":"Data from AI before filtering","data":{"ai_keys":list(data_analise.keys()),"ai_keys_count":len(data_analise.keys())},"timestamp":int(time.time()*1000)}) + '\n')
            # #endregion
            ...
            if expected_keys:
                ...
                for key in expected_keys:
                    ...
                    # #region agent log
                    with open(r'e:\Projetos Cursor\ProcessIA\processia\.cursor\debug.log', 'a', encoding='utf-8') as f:
                        f.write(json_module.dumps({"sessionId":"debug-session","runId":"run1","hypothesisId":"C","location":"worker_csv.py:process_file_task:find_key","message":"Searching for key in AI response","data":{"key":key,"found":value is not None,"value_preview":str(value)[:50] if value else None},"timestamp":int(time.time()*1000)}) + '\n')
                    # #endregion
                ...
                # #region agent log
                with open(r'e:\Projetos Cursor\ProcessIA\processia\.cursor\debug.log', 'a', encoding='utf-8') as f:
                    f.write(json_module.dumps({"sessionId":"debug-session","runId":"run1","hypothesisId":"D","location":"worker_csv.py:process_file_task:after_filter","message":"Data after filtering","data":{"filtered_keys":list(filtered_data.keys()),"filtered_count":len(filtered_data)},"timestamp":int(time.time()*1000)}) + '\n')
                # #endregion
